...
function Card({
  ...
  return (
    <div
      ...
      className={cn(
        "morf-card-surface group/card text-card-foreground ring-foreground/5 flex flex-col gap-(--card-spacing) overflow-hidden rounded-4xl border border-transparent py-(--card-spacing) text-sm ring-1 transition-[box-shadow,background-color,border-color] [--card-spacing:--spacing(6)] has-[>img:first-child]:pt-0 data-[interactive=true]:cursor-pointer data-[interactive=true]:outline-none data-[interactive=true]:focus-visible:ring-3 data-[interactive=true]:focus-visible:ring-ring/30 data-[selected=true]:border-ring data-[selected=true]:shadow-none data-[selected=true]:ring-0 data-[size=sm]:[--card-spacing:--spacing(4)] motion-reduce:transition-none *:[img:first-child]:rounded-t-4xl *:[img:last-child]:rounded-b-4xl",
        className,
      )}
      {...props}
    />
